...
def detect_ball_position(img_r, img_l):
    ...
    focal_length = 512 / 2  # Camera focal length in pixels (adjust as per your camera specs)
    ...
    # Assume that the image plane's origin is at the center of the image
    cx, cy = 256, 256  # Image center (half of the image width/height)
    x = (pos_l[0] - cx) * z / focal_length  # Convert x-coordinate
    y = (pos_l[1] - cy) * z / focal_length  # Convert y-coordinate
    
    print(f"Detected Ball Position (x, y, z): ({x}, {y}, {z})")  # Print final 3D coordinates
    return (x, y, z)
